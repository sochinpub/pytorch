...
def do_send_recv(rank, world_size):
    os.environ["nccl_SEND_RECV_ENABLE"] = str(1)
    ...
    p2p_op_list = []
    if rank == 0:
        send_tensor = torch.randn(10).to("cuda:{}".format(rank))
        send_op = dist.P2POp(dist.isend, send_tensor, 1)
        p2p_op_list.append(send_op)
    elif rank == 1:
        recv_tensor = torch.randn(10).to("cuda:{}".format(rank))
...
def do_allgather(rank, world_size):
    ...
    t = torch.randn(10).to("cuda")
    ts = [torch.randn(10).to("cuda") for i in range(2)]
    # allgather operation
